...
async def _get_store(config: AppConfig) -> EventStore:
    ...
    try:
        ...
        pass  # schema already exists, skip if locked by concurrent process
    return store
...
async def _process_watch(name: str, interval: int, config_path: str | None, cycle_fn, max_queries: int = 10, results_per_query: int = 5):
    ...
    from helioryn.cache import CH_PIPELINE, CH_SCORER, CH_ANALYZER, CH_INTERPRETER, HeliorynCache
    _TRIGGER_MAP = {
        "scorer": CH_PIPELINE,
        "analyzer": CH_SCORER,
...
async def _api_ingest_cycle(config_path: str | None, _log=None):
    """Run one API ingestion cycle — query all configured API data sources."""
    from helioryn.config import AppConfig
    from helioryn.store import EventStore
    ...
    from helioryn.ingest.api_source import create_api_sources

    ingestor = PostgresIngestor(store, config.ingest.domain_denylist if config.ingest else [])
    api_configs = getattr(config.ingest, "api_sources", []) if config.ingest else []
    if isinstance(api_configs, dict):
        api_configs = list(api_configs.values())
    sources = create_api_sources(api_configs, ingestor, store)
    if not sources:
        typer.echo("No API sources configured.")
        await store.close()
        return

    for src in sources:
        typer.echo(f"  Running {src.name}...")
        result = await src.run_cycle()
        typer.echo(f"    Fetched: {result['fetched']}, Ingested: {result['ingested']}, Skipped: {result['skipped']}, Errors: {result['errors']}")

    await store.close()


@app.command("api-ingest")
def api_ingest_cmd(
    config_path: str | None = typer.Option(None, "--config", "-c"),
):
    """Run one API ingestion cycle."""
    asyncio.run(_api_ingest_cycle(config_path))
            ...
            try:
                ...
                for row in rows:
                    ...
                    try:
                        ...
                        if not topic:
                            topic = (snapshot.metadata or {}).get("topic", "")
...
def daemon(
    ...
    asyncio.run(_daemon_manager(interval, max_queries, results_per_query, config_path, no_pipeline, no_scorer, no_analyzer, no_interpreter))
